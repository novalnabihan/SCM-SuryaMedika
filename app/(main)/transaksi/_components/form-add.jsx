...
export const FormAdd = ({ onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!warehouseId) {
      showAlert({ severity: "error", message: "Gudang wajib dipilih." });
      setLoading(false);
      return;
    }

    if (!partner) {
      showAlert({ severity: "error", message: "Nama partner wajib diisi." });
      setLoading(false);
      return;
    }
    ...
    if (invalidItem) {
      showAlert({
        severity: "error",
        message:
          "Item tidak valid. Pastikan semua item memiliki barang, jumlah, dan harga yang benar.",
      });
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        showAlert({
          severity: "info",
          message: "Transaksi telah tercatat dan stok telah diperbarui.",
        });
      ...
      showAlert({
        severity: "error",
        message: "Terjadi kesalahan jaringan saat mencatat transaksi.",
      });
    } finally {
      setLoading(false);
    }
