...
export default function ModalExportFilter({ transactions = [] }) {
  ...
  const filterOptions = useMemo(() => {
    const invoiceCodes = [...new Set(transactions.map(t => t.invoiceCode))];
    const itemIds = [...new Set(transactions.map(t => t.itemId))];
    const itemNames = [...new Set(transactions.map(t => t.itemName))];
    const operators = [...new Set(transactions.map(t => t.operator))];
    const partners = [...new Set(transactions.map(t => t.partner).filter(Boolean))];
    return { invoiceCodes, itemIds, itemNames, operators, partners };
  }, [transactions]);
  ...
  const applyFilters = () => {
    return transactions.filter(trx => {
      return (
        ...
        (filters.itemId ? trx.itemId === filters.itemId : true) &&
...
const handleExport = async () => {
    ...
    if (filteredCount === 0) {
      alert("Tidak ada data yang cocok dengan filter Anda. Tidak ada file yang akan diekspor.");
    ...
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/warehouses/export-csv/filtered`, {
        method: 'POST',
      ...
      let filename = `export-terfilter.csv`;
